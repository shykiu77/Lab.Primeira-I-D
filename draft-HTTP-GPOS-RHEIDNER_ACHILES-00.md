...
# Title draft-RHEIDNER-ACHILES-00.txt
...
## 1. Introduction
Este trabalho pretende criar uma extensão que visa melhorar o protocolo HTTP. Tal melhora será feita com a utilização de um novo campo no protocolo HTTP, o qual servirá para requisitar a geolocalização (latitude, longitude e altitude) de algum site presente na Internet.
...
A requisição da geolocalização (latitude, longitude e altitude) de um site será feita da seguinte maneira: 
Uma mensagem GET HTTP deverá receber em seu cabeçalho do cliente mais um campo extra `GPOS` que deverá ser preenchido com o `nome_do_dispositivo`.
```
GET <arquivo> HTTP/1.1
HOST: <host>
GPOS: <nome_dispositivo>
```
O servidor deverá, então, responder uma mensagem que contenha o formato:
